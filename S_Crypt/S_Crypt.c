...
char szProcessNameCur[NT_PROCNAMELEN] ;
...
BOOLEAN Ps_IsCurrentProcessMonitored(WCHAR* pwszFilePathName, ULONG uLength,BOOLEAN* bSystemAndExt)
{
	...
	try{
		...
		RtlZeroMemory(szProcessName,16);
		GetCurrentProcessName(szProcessName);

		// save file path name in local buffer ,May be to avoid BSOD ~
					   ...
					   )
{
	...
	try {
		...
		if (NULL != pFileGUID)
			ExFreePoolWithTag(pFileGUID, NAME_TAG) ;
	}

	return CheckRet;
}

//执行PosDirCtrl的判断，explorer.exe 就是发送此IRP来获取文件大小
			 ...
			 )
{
	...
	//CanFilter = TRUE;避免驱动一加载就自动启动加密处理~
...
{        
	...
	KdPrint( ("S_Crypt!S_Unload: Entered With FreeChain!\n") );
	FreeChain();
	CanFilter = FALSE;
			 ...
			 )
{
	...
	try{
		...
		*/
		...
		if(FileFlag2Flag == TRUE)
		{
			if (psFileFlag2 != NULL)//未释放导致的蓝屏
		   ...
		   )
{
	...
	try {		
		...
		} else {//如果系统不是用的上述两种读写方式，那么说明系统使用的是其他方式读写，此时在DPC的中断级别下是无法获取到正确的缓冲区地址，需要更改我们的IRQL；
			//
			   ...
			   )
{
	...
	try {
		//由于如果消息还在处理的时候，如果应用突然被关闭,会造成应用传入的InputBuffer内存无效，驱动访问这些内存错误,从而引起蓝屏，故这里拷贝一下
		...
		if (NULL != psSendType)
			ExFreePoolWithTag(psSendType, NAME_TAG) ;			
	}
						...
						)
{
	...
	__try 
	{
		pFileFlag = (PFILE_FLAG)ExAllocatePoolWithTag(NonPagedPool, FILE_FLAG_LENGTH, FILEFLAG_POOL_TAG1);
		...
		if (NULL != pFileFlag)
			ExFreePoolWithTag(pFileFlag, FILEFLAG_POOL_TAG1) ;		
...
// 供以后用于进程ID判断的参考，当MD5验证成功后就将进程ID加入有效的进程ID列表。
// BOOLEAN S_ProcValid(
//     HANDLE uProcID
//     )
// {
